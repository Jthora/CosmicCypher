...
//                }
        ...
        private static func fetchSelectedNodeTypes() -> [CoreAstrology.AspectBody.NodeType] {
            guard let decoded = UserDefaults.standard.data(forKey: _selectedNodeTypesKey) else {
            ...
            let allowedClassesSet = NSSet(array: allowedClasses) as! Set<AnyHashable>

            guard let rawValues = try? NSKeyedUnarchiver.unarchivedObject(ofClasses: allowedClassesSet, from: decoded) as? [NSNumber] else {
                print("Cannot unarchive object data: defaulting to DEFAULT_SELECTED_NODETYPES")
                defer { storeAspects() }
                return DEFAULT_SELECTED_NODETYPES
            }

            print("Decoded data: \(rawValues)")
            let values = rawValues.map { $0.intValue }
            let selectedPlanets = values.compactMap { CoreAstrology.AspectBody.NodeType(rawValue: $0) }
            return selectedPlanets
        }

        private static func fetchSelectedAspects() -> [CoreAstrology.AspectRelationType] {
            guard let decoded = UserDefaults.standard.data(forKey: _selectedAspectsKey) else {
            ...
            guard let rawValues = try? NSKeyedUnarchiver.unarchivedObject(ofClasses: allowedClassesSet, from: decoded) as? [NSNumber] else {
                print("Cannot unarchive object data: defaulting to DEFAULT_SELECTED_ASPECTS")
                return DEFAULT_SELECTED_ASPECTS
            }

            print("Decoded data: \(rawValues)")
            let values = rawValues.map { $0.intValue }
            let selectedAspects = values.compactMap { CoreAstrology.AspectRelationType(rawValue: $0) }
