...
extension ResonanceReportViewController {
    ...
    // MARK: UI Visibility Controls
    // Toogle UI Hidden
    func toggleCentralUI(animate:Bool = true) {
        DispatchQueue.main.async {
            self.interfaceIsHidden = !self.interfaceIsHidden
            self.hideCentralUI(hide: self.interfaceIsHidden)
        }
    }
    // Hide UI
    func hideCentralUI(hide:Bool = true, animate:Bool = true) {
        DispatchQueue.main.async {
            if animate {
                if hide {
