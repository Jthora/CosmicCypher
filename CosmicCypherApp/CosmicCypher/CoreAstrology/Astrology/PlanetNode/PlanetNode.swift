...
public class PlanetNode: Codable {
    ...
    public lazy var state:PlanetNodeState? = { return createState() }()
    ...
    // Create Motion State
    /// Motion State
    func createState(motionState:PlanetNodeState.MotionState? = nil) -> PlanetNodeState? {
        
        /// Planet State
        guard let planet = nodeType.planet(date: date) else { return nil }
        ...
        return PlanetNodeState(nodeType: nodeType,
                               date: date,
                               degrees: longitude,
